...
export async function sendCertificateChatMessage({apiKey, conversationId, senderId, employeeName, courseName, url}) {
  const res = await fetch(
    `https://api.connecteam.com/chat/v1/conversations/${conversationId}/message`,
    {
      ...
      body: JSON.stringify({
        ...
        text: `${employeeName} completed ${courseName}. Certificate: ${url}`
      })
    }
  );
